...
from .basic_utils import normalize
...
def plot_save(y, fname, is2d=False):
    
    plt.figure(figsize=(10,10))
    y_ = normalize(y if is2d else np.max(y, axis=0),pmin=2,pmax=99.8,clip=True)
    plt.axis("off")
    plt.margins(0,0)
    plt.subplots_adjust(top = 1, bottom = 0, right = 1, left = 0, hspace = 0, wspace = 0)
    plt.imsave(fname, np.squeeze(y_), cmap='magma')
...
def _plot_some(arr, title_list=None, pmin=0, pmax=100, cmap='magma', save_path=None, **imshow_kwargs):
    """
